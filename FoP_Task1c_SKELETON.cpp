...
int main()
{
	...
	//Item zombies[4] = { {1 , 1, zombie}, { 1 , SIZEY-2, zombie }, { SIZEX-2, 1, zombie }, { SIZEX-2 , SIZEY-2, zombie }};
	...
	do {
		...
		if (isArrowKey(key))
		{
			updateGameData(grid, spot, key, message, lifeText, lives, pillText, pillsLeft, zombies, freeze, kill, eat);		//move spot in that direction
			updateGrid(grid, maze, spot, zombies);					//update grid information
			/*for (int i(0); i <= 3; i++) {
				updateGrid(grid, maze, zombies[i]);
			}*/
		}
		else
...
{ //initialise grid and place spot in middle
	...
	void updateGrid(char g[][SIZEX], char m[][SIZEX], Item b, vector<Item>& zombies);
...
//TODO: initial maze configuration should be amended (size changed and inner walls removed)
  //initialise maze configuration
	...
	/*
	maze[1][1] = zombie;
	maze[1][SIZEX-2] = zombie;
	maze[SIZEY-2][1] = zombie;
	maze[SIZEY-2][SIZEX-2] = zombie;
	*/
	//holes amendment
	int x, y;
	for (int i(0); i <= 11; i++) {
...
//---------------------------------------------------------------------------
void updateGameData(const char g[][SIZEX], Item& spot, const int key, string& mess, string& lifeText, 
	int& lives, string& pillText, int& pillsLeft , vector<Item>& zombies, bool& freeze, bool& kill, bool& eat)
